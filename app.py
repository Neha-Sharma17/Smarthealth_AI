...
for k,v in [("logged_in",False),("user_name",""),("user_email",""),
            ...
            ("result",None),("chat_messages",[]),("emergency_shown",False)]:
...
def do_logout():
    st.session_state.update(logged_in=False,user_name="",user_email="",
                             ...
                             chat_messages=[],emergency_shown=False)
...
def main_app():
    ...
    # ── EXTREME CONDITION POPUP ───────────────────────────────────────────────
    is_extreme,extreme_msg=check_extreme_condition()
    if is_extreme and not st.session_state.emergency_shown:
        ...
        <div style="position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,.7);
          ...
          <div style="background:#1a1d28;border:2px solid #ff5f6d;border-radius:20px;
            padding:36px;max-width:440px;text-align:center">
            <div style="font-size:52px;margin-bottom:12px">🆘</div>
            <div style="font-family:'Syne',sans-serif;font-size:22px;color:#ff5f6d;margin-bottom:12px">
              We're concerned about you</div>
            <div style="font-size:14px;color:#8892b0;line-height:1.6;margin-bottom:20px">
              {extreme_msg}</div>
            <div style="font-size:13px;color:#eef0f8;margin-bottom:8px">
              📞 Vandrevala Foundation Helpline</div>
            <div style="font-family:'Syne',sans-serif;font-size:24px;color:#ff5f6d;margin-bottom:16px">
              1860-2662-345</div>
            <div style="font-size:12px;color:#5a6080">Available 24/7 · Free · Confidential</div>
          </div>
        </div>""",unsafe_allow_html=True)
        st.error(f"🆘 **Alert:** {extreme_msg}\n\n📞 **Vandrevala Helpline:** 1860-2662-345 (24/7 Free)")
    ...
    with selected[8]:
        ...
        # Extreme condition warning
        is_ex,ex_msg=check_extreme_condition()
        if is_ex:
            st.markdown(f'<div style="background:rgba(255,95,109,.1);border:2px solid #ff5f6d;border-radius:16px;padding:20px;margin-bottom:20px;text-align:center"><div style="font-size:32px;margin-bottom:8px">🚨</div><div style="font-family:\'Syne\',sans-serif;font-size:18px;color:#ff5f6d;margin-bottom:8px">Extreme Condition Detected</div><div style="font-size:13px;color:#8892b0">{ex_msg}</div><div style="font-size:14px;color:#eef0f8;margin-top:12px">Please call a helpline or your emergency contact immediately.</div></div>',unsafe_allow_html=True)
